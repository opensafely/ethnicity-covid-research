...
cap parmest, label eform format(estimate p lb ub) saving("$Tempdir/model4_`i'_eth16", replace) idstr("model4_`i'_eth16") 
...
cap logistic positivetest i.ethnicity_16 i.male age1 age2 age3 	i.imd						///
										...
										if carehome==1, nolog		
cap estimates save "$Tempdir/model5_`i'_eth16", replace
cap eststo model7

cap parmest, label eform format(estimate p lb ub) saving("$Tempdir/model5_`i'_eth16", replace) idstr("model5_`i'_eth16") 
local hr "`hr' "$Tempdir/model5_`i'_eth16" "



/* Estout================================================================*/ 
esttab model1 model2 model3 model4 model5  model6  using "$Tabfigdir/estout_table4_testedpop_eth16.txt", b(a2) ci(2) label wide compress eform ///
	title ("`i'") ///
	varlabels(`e(labels)') ///
	stats(N_sub) ///
	append 
