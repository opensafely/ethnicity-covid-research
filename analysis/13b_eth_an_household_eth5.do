/*==============================================================================
DO FILE NAME:			13b_eth_an_household_eth5
...
cap log close
macro drop hr
log using $logdir\13b_eth_an_household_eth5, replace t 

cap file close tablecontent
file open tablecontent using $Tabfigdir/table7_eth5.txt, write text replace
...
estimates save "$Tempdir/hh_crude_`i'_eth5_`eth'", replace 
parmest, label eform format(estimate p lb ub) saving("$Tempdir/hh_crude_`i'_eth5_`eth'", replace) idstr("hh_crude_`i'_eth5_`eth'") 
local hr "`hr' "$Tempdir/hh_crude_`i'_eth5_`eth'" "

/* Multivariable hh_models */ 
*Age and gender
stcox i.hh_total_cat i.male age1 age2 age3, strata(stp)
estimates save "$Tempdir/hh_model0_`i'_eth5_`eth'", replace 
parmest, label eform format(estimate p lb ub) saving("$Tempdir/hh_model0_`i'_eth5_`eth'", replace) idstr("hh_model0_`i'_eth5_`eth'")
local hr "`hr' "$Tempdir/hh_model0_`i'_eth5_`eth'" " 
...
if _rc==0{
estimates
estimates save "$Tempdir/hh_model1_`i'_eth5_`eth'", replace 
parmest, label eform format(estimate p lb ub) saving("$Tempdir/hh_model1_`i'_eth5_`eth'", replace) idstr("hh_model1_`i'_eth5_`eth'")
local hr "`hr' "$Tempdir/hh_model1_`i'_eth5_`eth'" " 
}
...
parmest, label eform format(estimate p lb ub) saving("$Tempdir/hh_model2_`i'_eth5_`eth'", replace) idstr("hh_model2_`i'_eth5_`eth'") 
local hr "`hr' "$Tempdir/hh_model2_`i'_eth5_`eth'" "
}
...
forvalues hh=1/3 {
	...
	cap estimates use "$Tempdir/hh_crude_`i'_eth5_`eth'" 
	cap cap lincom `hh'.hh_total_cat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab %4.2f (r(lb)) (" - ") %4.2f (r(ub)) _tab 
	cap estimates clear
	cap estimates use "$Tempdir/hh_model0_`i'_eth5_`eth'" 
	cap cap lincom `hh'.hh_total_cat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab %4.2f (r(lb)) (" - ") %4.2f (r(ub)) _tab 
	cap estimates clear
	cap estimates use "$Tempdir/hh_model1_`i'_eth5_`eth'" 
	cap cap lincom `hh'.hh_total_cat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab %4.2f (r(lb)) (" - ") %4.2f (r(ub)) _tab 
	cap estimates clear
	cap estimates use "$Tempdir/hh_model2_`i'_eth5_`eth'" 
	cap cap lincom `hh'.hh_total_cat, eform
...
drop parm  stderr z 
order outcome model eth5 hh_cat
...
outsheet using "$Tabfigdir/FP_household_eth5.txt", replace


* Close log file 
log close

insheet using $Tabfigdir/table7_eth5.txt, clear
insheet using "$Tabfigdir/FP_household_eth5.txt", clear
