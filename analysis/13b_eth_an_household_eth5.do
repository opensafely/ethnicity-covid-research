...
stcox i.hh_total_cat, strata(stp) nolog
...
forvalues hh=2/4 {
	...
	file write tablecontent  ("`lab`hh''") _tab (`denominator') _tab  (`event') _tab %10.0f (`person_week') _tab %3.2f (`rate') _tab  
	 estimates use "$Tempdir/hh_crude_`i'_eth5_`eth'" 
	 lincom `hh'.hh_total_cat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab ("(") %4.2f (r(lb)) (" - ") %4.2f (r(ub)) (")") _tab 
	 estimates clear
	 estimates use "$Tempdir/hh_model0_`i'_eth5_`eth'" 
	 lincom `hh'.hh_total_cat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab ("(") %4.2f (r(lb)) (" - ") %4.2f (r(ub)) (")") _tab 
	 estimates clear
	 estimates use "$Tempdir/hh_model1_`i'_eth5_`eth'" 
	 lincom `hh'.hh_total_cat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab ("(") %4.2f (r(lb)) (" - ") %4.2f (r(ub)) (")") _tab 
	 estimates clear
	 estimates use "$Tempdir/hh_model2_`i'_eth5_`eth'" 
	 lincom `hh'.hh_total_cat, eform
