...
file write tablecontent _tab ("Denominator") _tab ("Total person-weeks") _tab ("Rate per 1,000") _tab ("Crude") _tab _tab ("Age/Sex Adjusted") _tab _tab ("Age/Sex/IMD Adjusted") _tab _tab 	("+ co-morbidities") _tab _tab _n
...
* First row, hh_cat =  1 (1-2 ppl) reference cat
	qui safecount if hh_total_cat==1
...
forvalues hh=2/3 {
	qui safecount if hh_total_cat==`hh'
	local denominator = r(N)
...
insheet using $Tabfigdir/table7_hh_eth5.txt, clear
