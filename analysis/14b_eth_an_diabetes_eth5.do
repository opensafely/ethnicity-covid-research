...
file open tablecontent using $Tabfigdir/table8_eth5.txt, write text replace
file write tablecontent ("Table 6: Ethnicity and diabetes type - Complete Case Analysis") _n
...
parmest, label eform format(estimate p lb ub) saving("$Tempdir/dm_crude_`i'_eth5_`eth'", replace) idstr("dm_crude_`i'_eth5_`eth'") 
...
forvalues hh=1/3 {
	...
	cap estimates use "$Tempdir/dm_crude_`i'_eth5_`eth'" 
	cap cap lincom `hh'.diabcat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab %4.2f (r(lb)) (" - ") %4.2f (r(ub)) _tab 
	cap estimates clear
	cap estimates use "$Tempdir/dm_model0_`i'_eth5_`eth'" 
	cap cap lincom `hh'.diabcat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab %4.2f (r(lb)) (" - ") %4.2f (r(ub)) _tab 
	cap estimates clear
	cap estimates use "$Tempdir/dm_model1_`i'_eth5_`eth'" 
	cap cap lincom `hh'.diabcat, eform
	file write tablecontent  %4.2f (r(estimate)) _tab %4.2f (r(lb)) (" - ") %4.2f (r(ub)) _tab 
	cap estimates clear
	cap estimates use "$Tempdir/dm_model2_`i'_eth5_`eth'" 
	cap cap lincom `hh'.diabcat, eform
...
drop parm  stderr z 
order outcome model eth5 dm_cat
...
insheet using $Tabfigdir/table8_eth5.txt, clear
insheet using "$Tabfigdir/FP_diabetes_eth5.txt", clear
